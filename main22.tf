...
locals {
  prefix   = "mvp"
  rg_name  = "tempo_poc_rg" #An existing Resource Group for the Application Gateway 
  ...
  subnet_name = "tempo-waf-net"
  vnet_name   = "tempo-poc-vnet"
...
resource "azurerm_key_vault" "agw" {
  name                       = "${local.prefix}-hub-kv1-tempo"
  location                   = data.azurerm_resource_group.rg.location
  resource_group_name        = data.azurerm_resource_group.rg.name
